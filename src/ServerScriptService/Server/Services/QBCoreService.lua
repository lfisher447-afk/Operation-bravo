...
-- Third-party detections integrated cleanly
...
function QBCoreService.new()
    return setmetatable({ Name = 'QBCoreService', _sessions = {}, _bin = Janitor.new(), _endpoints = nil, _clock = 0 }, QBCoreService)
end
function QBCoreService:attach(player: Player)
    ...
    player.CharacterAdded:Connect(function(char)
        StateCheck:characterAdded({ player = player, strike = function(_, val, msg) self:strike(player, val, msg) end }, char)
        ToolCheck:characterAdded({ player = player, strike = function(_, val, msg) self:strike(player, val, msg) end }, char)
    end)
end
function QBCoreService:detach(player: Player)
    ...
    if session then session:destroy(); self._sessions[player] = nil end
end
...
function QBCoreService:tick(dt: number)
    ...
    for player, session in pairs(self._sessions) do
        ...
        local simulatedPlayer = { player = player, strike = function(_, val, msg) self:strike(player, val, msg) end }
        RotationCheck:checkPlayer(simulatedPlayer)

        local v = MovementChecks.run(player, session, sample)
        if v.sev > 0 then
            session.score += v.sev
            if v.correct then session.punish += v.sev end
            Violations.report(player, v)
            if Cfg.kickEnabled and now >= session.pauseUntil and session.punish >= Cfg.kickScore then
                player:Kick(Cfg.kickMessage); continue
            end
            if fixRemote and Reconciler.apply(session, sample, v, fixRemote) then
                session.buf:wipe(); session.speedWin:wipe(); session.score = 0; session.airTime = 0; session.hoverFrames = 0; session.speedStreak = 0; continue
            end
        else
...
function QBCoreService:Start()
    ...
    self._bin:hook(eps.sample.OnServerEvent:Connect(function(player: Player, payload: any) self:onClientPacket(player, payload) end))
    self._bin:hook(Players.PlayerAdded:Connect(function(player: Player) self:attach(player) end))
    self._bin:hook(Players.PlayerRemoving:Connect(function(player: Player) self:detach(player) end))
    ...
    for _, player in ipairs(Players:GetPlayers()) do task.spawn(function() self:attach(player) end) end
end
