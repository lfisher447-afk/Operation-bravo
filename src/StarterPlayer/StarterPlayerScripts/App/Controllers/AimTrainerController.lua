...
local function createTarget(parent: Instance, pos: Vector3, categoryName: string)
    ...
    part.Name = "AimTrainerTarget"
    part.Size = Vector3.new(2, 2, 2)
    part.Shape = Enum.PartType.Ball
    part.Color = Color3.fromRGB(0, 255, 65)
    part.Material = Enum.Material.Neon
    part.Position = pos
    part.CanCollide = false
    part.Anchored = true
    part:SetAttribute("Category", categoryName)
    part.Parent = parent
    
    local clickDetector = Instance.new('ClickDetector')
    clickDetector.MaxActivationDistance = 300
...
function AimTrainerController:RegisterRaycastHit(target: Instance, hitPosition: Vector3)
    local categoryName = target:GetAttribute("Category") or "Gridshot"
    target:Destroy()
    local remotes = SharedApp:WaitForChild('Remotes')
    local trainHit = remotes:WaitForChild('TrainHit') :: RemoteEvent
    trainHit:FireServer(categoryName)
